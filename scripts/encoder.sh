...
SECOND=$(env time --format="Sec %e" bash -c " $BASE --pass=2 $FLAGS --fpf=$OUTPUT/$FOLDER/$FOLDER.log -o $OUTPUT/$FOLDER/$FOLDER.mkv - 2>&1" 2>&1 | awk ' /Sec/ { print $2 }') &&
rm -f "$OUTPUT/$FOLDER/$FOLDER".log &&
SIZE=$(du "$OUTPUT/$FOLDER/$FOLDER".mkv | awk '{print $1}') &&
